...
/* package */ final class RtspMessageUtil {
  ...
  public static ImmutableList<String> serializeRequest(RtspRequest request) {
    ...
    builder.add(
        Util.formatInvariant(
            "%1$s %s %2$s", toMethodString(request.method), request.uri, RTSP_VERSION));
    ...
    for (String headerName : headers.keySet()) {
      ...
      for (int i = 0; i < headerValuesForName.size(); i++) {
        builder.add(Util.formatInvariant("%1$s: %2$s", headerName, headerValuesForName.get(i)));
      }
    }
  ...
  public static ImmutableList<String> serializeResponse(RtspResponse response) {
    ...
    builder.add(
        Util.formatInvariant(
            "%1$s %s %2$s", RTSP_VERSION, response.status, getRtspStatusReasonPhrase(response.status)));
