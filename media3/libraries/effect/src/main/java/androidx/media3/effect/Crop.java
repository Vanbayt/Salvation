...
public final class Crop implements MatrixTransformation {
  ...
  public Crop(float left, float right, float bottom, float top) {
    checkArgument(right > left, "right value %1$s should be greater than left value %2$s", right, left);
    checkArgument(top > bottom, "top value %1$s should be greater than bottom value %2$s", top, bottom);
    this.left = left;
    this.right = right;
    this.bottom = bottom;
    this.top = top;
