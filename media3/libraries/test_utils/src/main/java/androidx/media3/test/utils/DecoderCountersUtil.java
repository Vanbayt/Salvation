...
public final class DecoderCountersUtil {
  ...
  public static void assertSkippedOutputBufferCount(
    ...
    assertWithMessage(
            "Codec(%1$s) skipped an unexpected number of buffers. Counters:\n%2$s", name, counters)
        .that(actual)
        .isEqualTo(expected);
  }
  ...
  public static void assertTotalBufferCount(
    ...
    assertWithMessage("Codec(%1$s) output too few buffers. Counters:\n%2$s", name, counters)
        .that(actual)
        .isAtLeast(minCount);
    assertWithMessage("Codec(%1$s) output too many buffers. Counters:\n%2$s", name, counters)
        .that(actual)
        .isAtMost(maxCount);
  }
  ...
  public static void assertDroppedBufferLimit(String name, DecoderCounters counters, int limit) {
    ...
    assertWithMessage(
            "Codec(%1$s) was late decoding too many buffers. Counters:\n%2$s: ", name, counters)
        .that(actual)
        .isAtMost(limit);
  }
  ...
  public static void assertConsecutiveDroppedBufferLimit(
    ...
    assertWithMessage(
            "Codec(%1$s) was late decoding too many buffers consecutively. Counters:\n%2$s",
            name, counters)
        .that(actual)
        .isAtMost(limit);
  }
  ...
  public static void assertVideoFrameProcessingOffsetSampleCount(
    ...
    assertWithMessage(
            "Codec(%1$s) videoFrameProcessingOffsetSampleCount too low. Counters:\n%2$s",
            name, counters)
        .that(actual)
        .isAtLeast(minCount);
    assertWithMessage(
            "Codec(%1$s) videoFrameProcessingOffsetSampleCount too high. Counters:\n%2$s",
            name, counters)
        .that(actual)
        .isAtMost(maxCount);
  }
