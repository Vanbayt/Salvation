...
// 🔥 ДОБАВЛЕННЫЕ ИМПОРТЫ ДЛЯ СЕТИ
import okhttp3.OkHttpClient
import java.util.concurrent.TimeUnit
// Обрати внимание: для Coil 3 может потребоваться импорт coil3.network.okhttp.okHttpClient,
// если студия его попросит, но обычно он подтягивается сам, если подключена зависимость coil-network-okhttp.
...
//            )
    ...
    override fun newImageLoader(context: PlatformContext): ImageLoader {
        return ImageLoader.Builder(context)
            ...
            .diskCache(null)
            .components {
                // 🔥 ВОТ ОНО! В Coil 3 чистый клиент добавляется сюда:
                add(
                ...
                // Дальше идут твои локальные парсеры для обложек с телефона (без изменений)
