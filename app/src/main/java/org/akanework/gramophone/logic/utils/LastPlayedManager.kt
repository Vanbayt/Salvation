...
class LastPlayedManager(
    ...
    fun save() {
        ...
        // 🔥 ИСПРАВЛЕНИЕ: Безопасная проверка типа вместо жесткого 'as'
        val currentShuffleOrder = controller.exoPlayer.shuffleOrder
        val persistent = if (shuffleModeEnabled && currentShuffleOrder is CircularShuffleOrder) {
            CircularShuffleOrder.Persistent(currentShuffleOrder)
        } else {
            null
        }
