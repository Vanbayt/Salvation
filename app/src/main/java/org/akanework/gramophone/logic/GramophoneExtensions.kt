...
fun MediaItem.getFile(): File? {
    ...
    // --- ИСПРАВЛЕНИЕ: Если это интернет, возвращаем null (файла нет) ---
...
fun MediaItem.getBitrate(): Int? {
    // --- ИСПРАВЛЕНИЕ (START) ---
    // Если это интернет-ссылка (http/https), мы не можем открыть её как файл.
    // Просто возвращаем null (битрейт неизвестен), чтобы не было краша.
    ...
    // Обернем получение файла в try-catch для надежности
    val file = try {
        getFile()
    } catch (e: IllegalArgumentException) {
        return null
