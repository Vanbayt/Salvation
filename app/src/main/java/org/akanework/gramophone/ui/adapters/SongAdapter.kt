...
class SongAdapter(
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int, payloads: MutableList<Any>) {
        // --- 1. НОВАЯ ЛОГИКА: ОПТИМИСТИЧНОЕ СЕРДЕЧКО ---
        // Ставим это в начало, чтобы сердечко всегда работало, даже если трек не играет
        val btnLike = holder.itemView.findViewById<android.widget.ImageButton>(R.id.btn_menu)
        btnLike?.let { button ->
            val item = getSongList()[position]
            var isLiked = false // Временная заглушка, пока не подключим БД

            updateLikeIcon(button, isLiked)
        ...
        // --- 2. ТВОЯ ОРИГИНАЛЬНАЯ ЛОГИКА АНИМАЦИИ ВОСПРОИЗВЕДЕНИЯ ---
    ...
    // Вспомогательная функция для смены иконок и анимации
    private fun updateLikeIcon(button: android.widget.ImageButton, isLiked: Boolean) {
        val iconRes = if (isLiked) {
            R.drawable.sl_check_fav // Закрашенное сердечко
        } else {
            R.drawable.ic_heart // Пустое сердечко
        }
        button.setImageResource(iconRes)

        // Красим в цвет акцента, если лайкнуто
        if (isLiked) {
            button.setColorFilter(androidx.core.content.ContextCompat.getColor(context, R.color.sl_fav_button))
        } else {
            button.clearColorFilter() // Сброс до стандартного цвета
        }
    ...
    // ВОССТАНОВЛЕННЫЙ БЛОК СОРТИРОВКИ (Ты случайно его удалил)
