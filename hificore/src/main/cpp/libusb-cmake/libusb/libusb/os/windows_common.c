...
unsigned long htab_hash(const char *str)
{
	...
	if (htab_table[idx].used) {
		...
		usbi_dbg(NULL, "hash collision ('%1$s' vs '%2$s')", str, htab_table[idx].str);
...
static enum windows_version get_windows_version(void)
{
	...
	if (vi.wServicePackMinor)
		usbi_dbg(NULL, "Windows %1$s SP%u.%u %2$s", w, vi.wServicePackMajor, vi.wServicePackMinor, arch);
	else if (vi.wServicePackMajor)
		usbi_dbg(NULL, "Windows %1$s SP%u %2$s", w, vi.wServicePackMajor, arch);
	else
		usbi_dbg(NULL, "Windows %1$s %2$s", w, arch);
