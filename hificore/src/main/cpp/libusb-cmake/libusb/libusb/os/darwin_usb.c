...
static int submit_bulk_transfer(struct usbi_transfer *itransfer) {
  ...
  if (kIOReturnSuccess != ret) {
    usbi_err (TRANSFER_CTX (transfer), "bulk transfer failed (dir = %1$s): %2$s (code = 0x%08x)", IS_XFERIN(transfer) ? "In" : "Out",
...
static int submit_stream_transfer(struct usbi_transfer *itransfer) {
  ...
  if (ret)
    usbi_err (TRANSFER_CTX (transfer), "bulk stream transfer failed (dir = %1$s): %2$s (code = 0x%08x)", IS_XFERIN(transfer) ? "In" : "Out",
               darwin_error_str(ret), ret);
...
static int submit_iso_transfer(struct usbi_transfer *itransfer) {
  ...
  if (kresult != kIOReturnSuccess) {
    usbi_err (TRANSFER_CTX (transfer), "isochronous transfer failed (dir: %1$s): %2$s", IS_XFERIN(transfer) ? "In" : "Out",
