	...
	HDEVINFO *dev_info, SP_DEVINFO_DATA *dev_info_data, const char *enumerator, unsigned _index)
{
	if (_index == 0) {
		*dev_info = pSetupDiGetClassDevsA(NULL, enumerator, NULL, DIGCF_PRESENT|DIGCF_ALLCLASSES);
		if (*dev_info == INVALID_HANDLE_VALUE) {
			usbi_err(ctx, "could not obtain device info set for PnP enumerator '%1$s': %2$s",
	...
	if (!pSetupDiEnumDeviceInfo(*dev_info, _index, dev_info_data)) {
		if (GetLastError() != ERROR_NO_MORE_ITEMS)
			usbi_err(ctx, "could not obtain device info data for PnP enumerator '%1$s' index %u: %2$s",
	...
	PSP_DEVINFO_DATA dev_info_data, LPCGUID guid, DWORD *_index, char **dev_interface_path)
{
	...
	for (;;) {
		if (!pSetupDiEnumDeviceInfo(dev_info, *_index, dev_info_data)) {
			if (GetLastError() != ERROR_NO_MORE_ITEMS) {
				usbi_err(ctx, "Could not obtain device info data for %1$s index %lu: %2$s",
		...
		if (GetLastError() != ERROR_NO_MORE_ITEMS) {
			usbi_err(ctx, "Could not obtain interface data for %1$s devInst %lX: %2$s",
	...
	if (!pSetupDiGetDeviceInterfaceDetailA(dev_info, &dev_interface_data, NULL, 0, &size, NULL)) {
		...
		if (GetLastError() != ERROR_INSUFFICIENT_BUFFER) {
			usbi_err(ctx, "could not access interface data (dummy) for %1$s devInst %lX: %2$s",
				guid_to_string(guid, guid_string), ULONG_CAST(dev_info_data->DevInst), windows_error_str(0));
			return LIBUSB_ERROR_OTHER;
		}
	} else {
	...
	if (!pSetupDiGetDeviceInterfaceDetailA(dev_info, &dev_interface_data,
		...
		usbi_err(ctx, "could not access interface data (actual) for %1$s devInst %lX: %2$s",
			guid_to_string(guid, guid_string), ULONG_CAST(dev_info_data->DevInst), windows_error_str(0));
...
static void cache_config_descriptors(struct libusb_device *dev, HANDLE hub_handle)
{
	...
	for (i = 0; i <= num_configurations; i++) {
		...
		if (!DeviceIoControl(hub_handle, IOCTL_USB_GET_DESCRIPTOR_FROM_NODE_CONNECTION, &cd_buf_short, size,
			...
			usbi_info(ctx, "could not access configuration descriptor %u (dummy) for '%1$s': %2$s", i, priv->dev_id, windows_error_str(0));
			continue;
		}
		...
		if (!DeviceIoControl(hub_handle, IOCTL_USB_GET_DESCRIPTOR_FROM_NODE_CONNECTION, cd_buf_actual, size,
			...
			usbi_err(ctx, "could not access configuration descriptor %u (actual) for '%1$s': %2$s", i, priv->dev_id, windows_error_str(0));
			continue;
		}
...
static int init_root_hub(struct libusb_device *dev)
{
	...
	if (handle == INVALID_HANDLE_VALUE) {
		usbi_err(ctx, "could not open root hub %1$s: %2$s", priv->path, windows_error_str(0));
		return LIBUSB_ERROR_ACCESS;
	}

	if (!DeviceIoControl(handle, IOCTL_USB_GET_NODE_INFORMATION, NULL, 0, &hub_info, sizeof(hub_info), &size, NULL)) {
		usbi_warn(ctx, "could not get root hub info for '%1$s': %2$s", priv->dev_id, windows_error_str(0));
		CloseHandle(handle);
		return LIBUSB_ERROR_ACCESS;
	}
	...
	if (windows_version >= WINDOWS_8) {
		...
		for (port_number = 1; port_number <= num_ports; port_number++) {
			...
			if (!DeviceIoControl(handle, IOCTL_USB_GET_NODE_CONNECTION_INFORMATION_EX_V2,
				...
				usbi_warn(ctx, "could not get node connection information (V2) for root hub '%1$s' port %lu: %2$s",
	...
	for (port_number = 1; port_number <= num_ports; port_number++) {
		...
		if (!DeviceIoControl(handle, IOCTL_USB_GET_NODE_CONNECTION_INFORMATION_EX, &conn_info, sizeof(conn_info),
			...
			usbi_warn(ctx, "could not get node connection information for root hub '%1$s' port %lu: %2$s",
				priv->dev_id, ULONG_CAST(port_number), windows_error_str(0));
			continue;
		}
	...
	uint8_t port_number, DEVINST devinst)
{
	...
	if (parent_dev != NULL) { // Not a HCD root hub
		...
		if (hub_handle == INVALID_HANDLE_VALUE) {
			usbi_warn(ctx, "could not open hub %1$s: %2$s", parent_priv->path, windows_error_str(0));
			return LIBUSB_ERROR_ACCESS;
		}
		...
		if (!DeviceIoControl(hub_handle, IOCTL_USB_GET_NODE_CONNECTION_INFORMATION_EX, &conn_info, sizeof(conn_info),
			&conn_info, sizeof(conn_info), &size, NULL)) {
			usbi_warn(ctx, "could not get node connection information for device '%1$s': %2$s",
		...
		if (windows_version >= WINDOWS_8) {
			...
			if (!DeviceIoControl(hub_handle, IOCTL_USB_GET_NODE_CONNECTION_INFORMATION_EX_V2,
				&conn_info_v2, sizeof(conn_info_v2), &conn_info_v2, sizeof(conn_info_v2), &size, NULL)) {
				usbi_warn(ctx, "could not get node connection information (V2) for device '%1$s': %2$s",
					priv->dev_id,  windows_error_str(0));
	...
	int *api, int *sub_api)
{
	...
	for (k = 0; k < 3; k++) {
		if (pSetupDiGetDeviceRegistryPropertyA(*dev_info, dev_info_data, lookup[k].reg_prop,
			...
			usbi_dbg(NULL, "%1$s(s): %2$s", lookup[k].designation, lookup[k].list);
		} else {
			if (GetLastError() != ERROR_INVALID_DATA)
				usbi_dbg(NULL, "could not access %1$s: %2$s", lookup[k].designation, windows_error_str(0));
			lookup[k].list[0] = 0;
		}
	}
	...
	for (i = 2; i < USB_API_MAX; i++) {
		for (k = 0; k < 3; k++) {
			...
			if (j >= 0) {
				usbi_dbg(NULL, "matched %1$s name against %2$s", lookup[k].designation,
					...
					int guid_number, GUID **if_guid)
{
	...
	*if_guid = malloc(sizeof(GUID));
	...
	if (!string_to_guid(guid, *if_guid)) {
		usbi_warn(ctx, "device '%1$s' has malformed DeviceInterfaceGUID string '%2$s', skipping", dev_id, guid);
		free(*if_guid);
		*if_guid = NULL;
		goto exit;
	}
...
#define UNREF_SIZE_STEP 16
	...
	for (pass = 0; ((pass < nb_guids) && (r == LIBUSB_SUCCESS)); pass++) {
		...
		usbi_dbg(ctx, "ENUM pass %1$s %2$s", passname[pass_type], guid_to_string(guid_list[pass], guid_string));
#endif
		...
		for (i = 0; ; i++) {
			...
			case HUB_PASS:
				...
				if (!pSetupDiGetDeviceRegistryPropertyA(*dev_info, &dev_info_data, SPDRP_ENUMERATOR_NAME,
					...
					usbi_err(ctx, "could not read enumerator string for device '%1$s': %2$s", dev_id, windows_error_str(0));
			...
			case EXT_PASS:
				...
				if ((!pSetupDiGetDeviceRegistryPropertyA(*dev_info, &dev_info_data, SPDRP_INSTALL_STATE,
					...
					usbi_warn(ctx, "could not detect installation state of driver for '%1$s': %2$s",
			...
			case GEN_PASS:
				...
				if (!get_dev_port_number(*dev_info, &dev_info_data, &port_nr))
					usbi_warn(ctx, "could not retrieve port number for device '%1$s': %2$s", dev_id, windows_error_str(0));
	...
	for (i = 0; i < USB_MAXINTERFACES; i++) {
		if ((priv->usb_interface[i].path != NULL)
			...
			if (file_handle == INVALID_HANDLE_VALUE) {
				usbi_err(HANDLE_CTX(dev_handle), "could not open device %1$s (interface %d): %2$s", priv->usb_interface[i].path, i, windows_error_str(0));
				switch (GetLastError()) {
					...
					*dev_interface_path_guid_start = '\0';
					...
					if (strncmp(dev_interface_path, priv->usb_interface[iface].path, strlen(dev_interface_path)) == 0) {
						...
						if (file_handle != INVALID_HANDLE_VALUE) {
							...
							usbi_err(ctx, "could not open device %1$s: %2$s", filter_path, windows_error_str(0));
						}
					}
				}
				if (r != LIBUSB_SUCCESS)
	...
	for (i = 0; i < USB_MAXINTERFACES; i++) {
		if ((priv->usb_interface[i].path != NULL)
			...
			if (hid_handle == INVALID_HANDLE_VALUE) {
				...
				if (hid_handle == INVALID_HANDLE_VALUE) {
					usbi_err(HANDLE_CTX(dev_handle), "could not open device %1$s (interface %d): %2$s", priv->path, i, windows_error_str(0));
					switch (GetLastError()) {
					case ERROR_FILE_NOT_FOUND: // The device was disconnected
						return LIBUSB_ERROR_NO_DEVICE;
					case ERROR_ACCESS_DENIED:
