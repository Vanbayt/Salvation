...
sunos_add_devices(di_devlink_t link, void *arg)
{
	...
	usbi_dbg(NULL, "device bus address=%1$s:%x, name:%2$s",
	...
	for (dn = di_child_node(myself); dn != DI_NODE_NIL; dn = di_sibling_node(dn)) {
		...
		usbi_dbg(NULL, "Device %1$s %2$s id=0x%" PRIx64 ", devcount:%" PRIuPTR
		    ", bdf=%" PRIx64,
    ...
    uint8_t ep_addr, int ep_type)
{
	...
	e = snprintf(filename, sizeof (filename), "%1$s/%sif%d%s%2$s%d",
	...
	if ((fd = open(filename, mode)) == -1) {
		usbi_dbg(HANDLE_CTX(hdl), "can't open %1$s: errno %d (%2$s)", filename, errno,
		    strerror(errno));
	...
	if ((ep_type == LIBUSB_TRANSFER_TYPE_INTERRUPT) &&
		...
		if ((fdstat = open(statfilename, O_RDWR)) == -1) {
			usbi_dbg(HANDLE_CTX(hdl), "can't open %1$s RDWR: errno %d (%2$s)",
			...
			if (count != 1) {
				...
				usbi_dbg(HANDLE_CTX(hdl), "can't write to %1$s: errno %d (%2$s)",
					statfilename, errno, strerror(errno));
				(void) close(fdstat);
		...
		if ((fdstat = open(statfilename, O_RDONLY)) == -1) {
			usbi_dbg(HANDLE_CTX(hdl), "can't open %1$s: errno %d (%2$s)", statfilename, errno,
				strerror(errno));
...
usb_do_io(struct libusb_context *ctx, int fd, int stat_fd, void *data, size_t size, int flag, int *status)
{
	...
	if (ret < 0) {
		...
		usbi_dbg(ctx, "TID=%x io %1$s errno %d (%2$s)", pthread_self(),
