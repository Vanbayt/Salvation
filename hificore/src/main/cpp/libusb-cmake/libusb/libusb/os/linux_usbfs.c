	...
	const char *sysfs_dir, const char *attr)
{
	...
	snprintf(filename, sizeof(filename), SYSFS_DEVICE_PATH "/%1$s/%2$s", sysfs_dir, attr);
	...
	const char *sysfs_dir, const char *attr, int max_value, int *value_p)
{
	...
	if (buf == endptr || value < 0 || value > (long)max_value || errno) {
		usbi_err(ctx, "attribute %1$s contains an invalid value: '%2$s'", attr, buf);
...
retry:
	...
	usbi_dbg(ctx, "dev %p (%1$s) has parent %p (%2$s) port %u",
