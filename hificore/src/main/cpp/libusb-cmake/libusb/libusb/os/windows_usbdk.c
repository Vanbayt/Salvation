...
static FARPROC get_usbdk_proc_addr(struct libusb_context *ctx, LPCSTR api_name)
{
	...
	if (api_ptr == NULL)
		usbi_err(ctx, "UsbDkHelper API %1$s not found: %2$s", api_name, windows_error_str(0));
	...
	PUSB_DK_DEVICE_ID id, unsigned long *session_id)
{
	...
	if (snprintf(dev_identity, sizeof(dev_identity), "%1$s%2$s", id->DeviceID, id->InstanceID) == -1) {
